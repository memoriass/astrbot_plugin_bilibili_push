...
class DatabaseManager:
    ...
    def _init_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """)
            conn.execute(
                "CREATE TABLE IF NOT EXISTS seen_dynamics (uid TEXT, dyn_id TEXT, PRIMARY KEY (uid, dyn_id))"
            )
            conn.execute(
    ...
    def remove_subscription(self, uid: str, sub_type: str, target_id: str) -> bool:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "DELETE FROM subscriptions WHERE uid = ? AND sub_type = ? AND target_id = ?",
    ...
    def get_subscriptions(self, target_id: Optional[str] = None) -> List[Subscription]:
        with sqlite3.connect(self.db_path) as conn:
            if target_id:
                cursor = conn.execute(
                    "SELECT uid, username, sub_type, target_id, categories, tags, enabled FROM subscriptions WHERE target_id = ?",
                    (target_id,),
                )
            else:
                cursor = conn.execute(
                    "SELECT uid, username, sub_type, target_id, categories, tags, enabled FROM subscriptions"
                )
    ...
    def is_seen_dynamic(self, uid: str, dyn_id: str) -> bool:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT 1 FROM seen_dynamics WHERE uid = ? AND dyn_id = ?",
                (str(uid), str(dyn_id)),
            )
    ...
    def add_seen_dynamic(self, uid: str, dyn_id: str):
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "INSERT OR IGNORE INTO seen_dynamics (uid, dyn_id) VALUES (?, ?)",
                (str(uid), str(dyn_id)),
            )
            conn.commit()
    ...
    def get_live_status(self, uid: str) -> tuple:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT last_status, last_title FROM live_status WHERE uid = ?",
                (str(uid),),
            )
            row = cursor.fetchone()
    ...
    def update_live_status(self, uid: str, status: int, title: str):
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "INSERT OR REPLACE INTO live_status (uid, last_status, last_title) VALUES (?, ?, ?)",
                (str(uid), status, title),
            )
            conn.commit()
