...
async def render_template(
    ...
    async with PageContext(viewport=viewport, device_scale_factor=2) as page:
        ...
        await page.wait_for_timeout(500) # 等待加载
        
        if selector == "body":
            return await page.screenshot(type="jpeg", full_page=True, quality=90)
        
        try:
            locator = page.locator(selector)
            return await locator.screenshot(type="jpeg", quality=90)
        except Exception:
            logger.warning(f"Selector {selector} not found, screenshotting full page")
            return await page.screenshot(type="jpeg", full_page=True, quality=90)
