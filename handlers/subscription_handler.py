...
class SubscriptionHandler:
        ...
        try:
            ...
            if self.db.add_subscription(sub):
                ...
                yield event.chain_result(
                    [
                        Comp.Plain(
                            f"✅ 已添加动态订阅: {user_info['username']} ({uid})"
                        ),
        ...
        try:
            ...
            if self.db.add_subscription(sub):
                ...
                yield event.chain_result(
                    [
                        Comp.Plain(
                            f"✅ 已添加直播订阅: {user_info['username']} ({uid})"
                        ),
                        Comp.Image.fromBytes(img_bytes),
                    ]
                )
            ...
            logger.error(f"Add live sub failed: {e}")
            yield event.plain_result(f"❌ 内部错误: {e}")
    ...
    async def list_subscriptions(self, event: AstrMessageEvent, scheduler):
        ...
        info_results = await asyncio.gather(
            *[fetch_info(uid) for uid in subs_map.keys()]
        )
        ...
        if live_uids:
            try:
                ...
                for i in live_infos:
                    live_status_map[str(i.uid)] = i.live_status == 1
            except Exception:
                pass
        ...
        try:
            ...
            logger.error(f"Render list failed: {e}")
