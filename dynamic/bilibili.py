...
class BilibiliDynamic(NewMessagePlatform):
    ...
    async def get_sub_list(self, target: Target) -> list[DynRawPost]:
        ...
        try:
            ...
            if "352" in err_msg or "412" in err_msg or "403" in err_msg:
                ...
                status_code = (
                    412 if "412" in err_msg else (352 if "352" in err_msg else 403)
                )
        ...
        if polymer_failed:
            try:
                ...
                if "412" in err_msg or "403" in err_msg or "352" in err_msg:
                    ...
                    logger.warning(
                        f"备用接口触发风控 ({status_code})，切换账号并重试..."
                    )
                    await HttpClient.invalidate_current_account(status_code=status_code)
                    if await HttpClient.rotate_account():
                        try:
    ...
    def _convert_fallback_card(self, desc: dict, card_json: dict) -> DynRawPost | None:
        ...
        try:
            ...
            author = PostAPI.Modules.Author(
                ...
                or get_any(card_json, "item.upload_time", "pubdate", "ctime")
                or 0,
            ...
            if dyn_type == "DYNAMIC_TYPE_AV":
                ...
                if pics:
                    ...
                    for pic in pics:
                        items.append(
                            DrawMajor.Item(
                                width=0, height=0, src=pic.get("img_src", "")
                            )
                        )
                    ...
                    try:
                        ...
                        if not orig_type:
                            if "aid" in origin_json:
                                ...
                                "item" in origin_json
                                and "pictures" in origin_json["item"]
                            ):
                                orig_type = 2
            ...
            logger.warning(
                f"Convert logic error ({e}) for card: {str(card_json)[:100]}..."
            )
            return None
        except Exception as e:
            logger.warning(
    ...
    def pre_parse_by_mojar(self, raw_post: DynRawPost) -> _ParsedMojarPost:
        ...
        if isinstance(major, VideoMajor):
            ...
            if not text:
                item_descs = [
                    item.description for item in major.draw.items if item.description
                ]
                if item_descs:
                    text = "\n".join(item_descs)
        ...
        return _ParsedMojarPost(
            "", desc, [], f"https://t.bilibili.com/{raw_post.id_str}"
        )
