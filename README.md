...
添加或删除订阅最终都需要用户引用确认卡片回复“确认”或“确认删除”。如果候选不明确，用户需要先引用候选卡片回复序号。
...
- `check_interval`: 基础检查间隔，默认 30 秒。
- `dynamic_check_interval`: 动态检查间隔，默认 120 秒，降低空间动态接口压力。
- `live_check_interval`: 直播检查间隔，默认 30 秒。
- `request_delay_sec`: 同一轮检查中不同 UP/批次之间的轻量间隔。
- `request_jitter_sec`: 每轮检查的随机抖动，避免固定整点集中请求。
- `live_batch_size`: 直播状态批量查询大小，默认 50。
- `risk_cooldown_sec`: 账号触发风控后的冷却时间，默认 1800 秒。
