...
- `entity_resolver.py`: UP 主实体解析入口，收集当前订阅/标签和历史别名候选，按置信度、来源层级和歧义边界选择。
...
- 用户选择搜索候选或确认订阅后，`entity_resolver.py` 会把本次查询词和 UID 写入 `up_aliases`；后续相同会话优先用历史别名直接命中对应 UP。
