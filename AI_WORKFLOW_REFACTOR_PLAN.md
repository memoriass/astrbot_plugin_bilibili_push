...
- 新增 `b站工作流` 显式命令。
- 新增后端 pending task 选择和确认流程；聊天侧使用引用回复续跑，不展示任务 ID。
...
- 复杂候选展示先返回卡片并创建后端 pending task，不直接写入订阅。
...
- 为 pending task 增加引用回复继续入口；短 ID 输入仅保留为兼容入口，例如 `bili<id> 1`、`bili<id> 确认`、`bili<id> 取消`。
...
- `search_up`: 搜索 UP 主，返回候选卡片并创建后端 pending task。
