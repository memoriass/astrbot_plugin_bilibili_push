...
- 默认配置面向生产稳定性，不追求最短延迟：动态 300 秒、直播 90 秒、基础兜底 60 秒。
- 每轮检查会加入最多 30 秒随机抖动，避免多实例或重启后固定时间集中请求。
- 同一轮不同动态 UP 或直播批次之间默认等待 1.5 秒。
- 账号触发风控后默认冷却 3600 秒，再恢复轮换。
- 用户可以在插件配置页调低间隔，但不建议在订阅量较大或账号池较少时使用激进值。
